...
public class DrawHelper {
    ...
    public void draw(FaceRectView faceRectView, List<DrawInfo> drawInfoList) {
        ...
        faceRectView.clearFaceInfo();//删除矩形
        if (drawInfoList == null || drawInfoList.size() == 0) {
            return;
        }
        for (DrawInfo drawInfo : drawInfoList) {
            Log.i("MSG","位置"+drawInfo.getRect());
            //调整矩形高度
            drawInfo.setRect(adjustRect(drawInfo.getRect(), previewWidth, previewHeight, canvasWidth, canvasHeight, 0, cameraId,
    ...
    public static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness) {
        ...
        mPath.moveTo(rect.left, rect.top + rect.height() / 6);
        mPath.lineTo(rect.left, rect.top);
        mPath.lineTo(rect.left + rect.width() / 6, rect.top);
        //右上
        mPath.moveTo(rect.right - rect.width() / 6, rect.top);
        mPath.lineTo(rect.right, rect.top);
        mPath.lineTo(rect.right, rect.top + rect.height() / 6);
        //右下
        mPath.moveTo(rect.right, rect.bottom - rect.height() / 6);
        mPath.lineTo(rect.right, rect.bottom);
        mPath.lineTo(rect.right - rect.width() / 6, rect.bottom);
        //左下
        mPath.moveTo(rect.left + rect.width() / 6, rect.bottom);
        mPath.lineTo(rect.left, rect.bottom);
        mPath.lineTo(rect.left, rect.bottom - rect.height() / 6);
